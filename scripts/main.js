...
let links = ["https://upload.wikimedia.org/wikipedia/pt/b/bd/Abaporu.jpg", "https://upload.wikimedia.org/wikipedia/commons/1/13/Michelangelo%2C_Creation_of_Adam_06.jpg", "../assets/comic-me.png", "https://upload.wikimedia.org/wikipedia/commons/thumb/0/0f/1665_Girl_with_a_Pearl_Earring.jpg/800px-1665_Girl_with_a_Pearl_Earring.jpg", "https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Vincent_van_Gogh_-_Self-Portrait_-_Google_Art_Project_%28719161%29.jpg/800px-Vincent_van_Gogh_-_Self-Portrait_-_Google_Art_Project_%28719161%29.jpg"];
...
algorithm.addEventListener('change', main);
...
async function main (ev) {
    ...
    if (!img || image.files[0] != img.src) {
        ...
        img.onload = function () {
            ...
            analyzeImage(img, algorithm.value == "deltaE94" ? deltaE94 : deltaE00);
        }
    ...
    function deltaE00 (a, b) {
        ...
        const deltaC = c2 - c1;
        const deltaH = Math.sqrt(deltaA * deltaA + deltaB * deltaB - deltaC * deltaC);
        const barH = (Math.abs(deltaA) + Math.abs(deltaB) - Math.abs(deltaH)) / 2;
        const T = 1 - 0.17 * Math.cos((barH - 30) * (Math.PI / 180)) + 0.24 * Math.cos(2 * barH * (Math.PI / 180)) + 0.32 * Math.cos((3 * barH + 6) * (Math.PI / 180)) - 0.20 * Math.cos((4 * barH - 63) * (Math.PI / 180));
        const deltaTheta = 30 * Math.exp(-((barH - 275) / 25) * ((barH - 275) / 25));
        const Rc = 2 * Math.sqrt(Math.pow(barC, 7) / (Math.pow(barC, 7) + Math.pow(25, 7)));
        const Sl = 1 + (0.015 * Math.pow(L1 - 50, 2)) / Math.sqrt(20 + Math.pow(L1 - 50, 2));
        const Sc = 1 + 0.045 * barC;
        const Sh = 1 + 0.015 * barC * T;
        const Rt = -Math.sin((2 * deltaTheta) * (Math.PI / 180)) * Rc;

        const deltaLKl = deltaL / (1 * Sl);
        const deltaCkc = deltaC / (1 * Sc);
        const deltaHkh = deltaH / (1 * Sh);

        const deltaE = Math.sqrt(Math.pow(deltaLKl, 2) + Math.pow(deltaCkc, 2) + Math.pow(deltaHkh, 2) + Rt * deltaCkc * deltaHkh);
...
download.addEventListener('click', downloadIt);
